...
package com.fynd.artoolkit.help;
...
import com.fynd.artoolkit.common.VisionImageProcessor;
...
public class CameraSource {
   ...
   * ASPECT_TOLERANCE : if the particular aspect ratio is not retrieved the select one within the tolerance range
   *
   *
   * @param size :Different preview size supported by device
   * @param desiredWidth the desired width of the camera preview frames
   * @param desiredHeight the desired height of the camera preview frames
   * @return the optimal preview size having same aspect ratio
   */
   ...
   * Preview sizes having same aspect ration are selected . If there is a corresponding picture size
  ...
  private static List<SizePair> generateValidPreviewSizeList(Camera camera) {
    ...
    for (Camera.Size pictureSize : supportedPictureSizes) {
      ...
      if (Math.abs(optimalSizeAspectRatio - pictureAspectRatio) < ASPECT_RATIO_TOLERANCE) {
        validPreviewSizes.add(new SizePair(optimalSize, pictureSize));
        break;
      }
    }
    if (validPreviewSizes.size() == 0) {
      Log.w(TAG, "No preview sizes have a corresponding same-aspect-ratio picture size");
      validPreviewSizes.add(new SizePair(optimalSize, null));
    }
